...
class TestVFSDiffDisplay:
    ...
    def test_new_file_shows_preview(self, capsys, tmp_path):
        ...
        result = vfs.write("new.py", "line1\nline2\nline3", "zh")
        captured = capsys.readouterr()
        assert result.is_ok()
        assert "new.py" in result.unwrap()
    ...
    def test_overwrite_shows_unified_diff(self, capsys, tmp_path):
        ...
        result = vfs.write("test.py", "old_a\nnew_b\nold_c\n", "zh")
        captured = capsys.readouterr()
        assert result.is_ok()
        output = captured.out
    ...
    def test_append_shows_preview(self, capsys, tmp_path):
        ...
        result = vfs.write("test.py", "appended\n", "zh", mode='a')
        captured = capsys.readouterr()
        assert result.is_ok()
        output = captured.out
    ...
    def test_binary_file_skips_diff(self, capsys, tmp_path):
        ...
        result = vfs.write("binary.bin", "new text", "zh")
        captured = capsys.readouterr()
        assert result.is_ok()
        output = captured.out
    ...
    def test_long_content_truncated(self, capsys, tmp_path):
        ...
        result = vfs.write("long.py", long_content, "zh")
        captured = capsys.readouterr()
        assert result.is_ok()
        output = captured.out
        # 预览应被截断
